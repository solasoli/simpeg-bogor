<?php
if(isset($_SESSION['id']) and isset($_SESSION['jab']))
{ 
...
<?php
$qc=mysqli_query($link,"select count(*) from pengaduan where status=0 and file_pengaduan is not null");
...
Daftar Laporan Pengaduan Aset Sekretariat Daerah
    <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
              <thead>
                ...
                <tbody>
                <?php
				$q1=mysqli_query($link,"select * from pengaduan inner join current_lokasi_kerja on current_lokasi_kerja.id_pegawai = pengaduan.id_pegawai where status=0");
				$i=1;
				$j=1;
...
$bagian=mysqli_fetch_array($qbag);
				if($data[6]!=NULL)
				{
				echo("<tr><td> $i</td> <td> $aset[0]</td><td><img id=myImg$j width=50 src=./gambar/$data[0].jpg /> </td><td> $data[4]</td><td>$peg[0]<br> $bagian[0] </td><td> $t1-$b1-$th1</td><td> $status <br>
				<a class='btn btn-danger' href=menu.php?x=respond.php&id=$data[0]>
                    Respond</a>
				...
				echo("<tr><td> $i</td> <td> $aset[0]</td><td></td><td> $data[4]</td><td>$peg[0]<br> $bagian[0] </td><td> $t1-$b1-$th1</td><td> $status <br>
				<a class='btn btn-danger' href=menu.php?x=respond.php&id=$data[0]>
                    Respond</a>
