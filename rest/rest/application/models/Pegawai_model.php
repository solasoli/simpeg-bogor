...
class Pegawai_model extends CI_Model{
    ...
    public function find_by_nama($nama){
        $sql = "SELECT p.id_pegawai, p.nama, p.nip_baru, p.jabatan, p.flag_pensiun, c.id_unit_kerja, u.nama_baru 
                ...
                WHERE p.flag_pensiun = 0 AND nama like '%$nama%'";
        return $this->db->query($sql);
    }
...
(SELECT a.id_pegawai, a.nip_baru AS nip, a.nama, a.tempat_lahir, a.tgl_lahir, a.usia, a.jenjab, a.pangkat_gol, a.jabatan,
    ...
    function listPegawaiAppsBisa($limit_awal, $limit_akhir, $keyApps){
        $sql = "SELECT b.nip_baru, b.nama, b.ponsel, b.email, b.password, b.foto, b.nip_enc, b.id_unit, b.unit,
                ...
                CONCAT("."'http://simpeg.kotabogor.go.id/simpeg/foto/'".", p.id_pegawai, '.jpg') as foto,
    ...
    public function daftar_pegawai_spesifik_kominfo(){
	    $sql = "SELECT b.* FROM
                (SELECT a.nip_baru AS nip, a.nama, a.pangkat, a.pangkat_gol, a.jenjab,
                   CASE WHEN a.eselon = 'Z' THEN 'Staf' ELSE a.eselon END AS eselon,
                   a.jabatan,
                a.unit, a.id_bos_atsl, a.tempat_lahir, a.tgl_lahir, a.status_nikah
                 FROM
                      ...
                      ROUND(DATEDIFF(CURRENT_DATE, DATE_FORMAT(p.tgl_lahir, '%Y/%m/%d'))/365,2) AS usia ,
                         ...
                         WHERE jap.id_jafung = jaf.id_jafung AND jap.id_pegawai = p.id_pegawai LIMIT 1) ELSE CASE WHEN p.id_j IS NULL THEN (SELECT jm.id_jfu AS kode_jabatan FROM jfu_pegawai jp, jfu_master jm
                         WHERE jp.kode_jabatan = jm.kode_jabatan AND jp.id_pegawai = p.id_pegawai LIMIT 1) ELSE j.id_j END END AS kode_jabatan,
                         ...
                         WHERE jap.id_jafung = jaf.id_jafung AND jap.id_pegawai = p.id_pegawai LIMIT 1) IS NULL THEN p.jabatan ELSE
                         (SELECT jaf.nama_jafung AS jabatan FROM jafung_pegawai jap, jafung jaf
                         WHERE jap.id_jafung = jaf.id_jafung AND jap.id_pegawai = p.id_pegawai LIMIT 1) END) ELSE CASE WHEN p.id_j IS NULL THEN (SELECT jm.nama_jfu AS jabatan FROM jfu_pegawai jp, jfu_master jm
                         WHERE jp.kode_jabatan = jm.kode_jabatan AND jp.id_pegawai = p.id_pegawai LIMIT 1) ELSE j.jabatan END END AS jabatan,
