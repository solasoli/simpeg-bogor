...
class RSA{
    ...
    */
    public function generate_keys ($p, $q, $show_debug=0){
        ...
        $m = substr($m,0,(strpos($m,'.')>0?strpos($m,'.'):strlen($m)));
        ...
        $n = substr($n,0,(strpos($n,'.')>0?strpos($n,'.'):strlen($n)));
        $e = substr($e,0,(strpos($e,'.')>0?strpos($e,'.'):strlen($e)));
        $d = substr($d,0,(strpos($d,'.')>0?strpos($d,'.'):strlen($d)));
