...
class Paginator{
	...
	function paginate()
	{
		...
		if($this->num_pages > 5)
		{
			...
			$this->return = ($this->current_page != 1 And $this->items_total >= 10) ? "<button id=\"btnPaging_previous\" onclick=\"pagingViewListLoad($prev_page,'$this->items_per_page$this->querystring');\" type='button'>&nbsp;&laquo; Previous&nbsp;</button>":"<button style=\"opacity: 0.6; pointer-events: none;\" type='button'> &laquo; Previous </button>";
			...
			{
				if($this->range[0] > 2 And $i == $this->range[0]) $this->return .= "<button type='button' style=\"opacity: 0.6; pointer-events: none;\"> ... </button>";
				...
				if($i==1 Or $i==$this->num_pages Or in_array($i,$this->range))
				{
					...
					$this->return .= ($i == $this->current_page And $_POST['page'] != 'All') ? "<button type='button' style=\"opacity: 0.6; pointer-events: none; background-color: orange;\">$i</button>":"<button type='button' id=\"btnPaging_$i\" onclick=\"pagingViewListLoad($i,'$this->items_per_page$this->querystring');\">&nbsp;$i&nbsp;</button>";
				}
				
				if($this->range[$this->mid_range-1] < $this->num_pages-1 And $i == $this->range[$this->mid_range-1]) $this->return .= "<button type='button' style=\"opacity: 0.6; pointer-events: none;\"> ... </button>";
			}
			...
			$this->return .= (($this->current_page != $this->num_pages And $this->items_total >= 10) And ($_POST['page'] != 'All')) ? "<button type='button' id=\"btnPaging_next\" onclick=\"pagingViewListLoad($next_page,'$this->items_per_page$this->querystring');\">&nbsp;Next &raquo;&nbsp;</button>\n":"<button type='button' style=\"opacity: 0.6; pointer-events: none;\">&raquo; Next</button>\n";
		...
		else
		{
			...
			for($i=1;$i<=$this->num_pages;$i++)
			{
				...
				$this->return .= ($i == $this->current_page) ? "<button type='button' style=\"opacity: 0.6; pointer-events: none; background-color: orange;\">$i</button> ":"<button type='button' id=\"btnPaging_$i\" onclick=\"pagingViewListLoad($i,'$this->items_per_page$this->querystring');\">&nbsp;$i&nbsp;</button> ";
	...
	function display_jump_menu()
	{
		...
		return "<span class=\"paginate\">Lompat ke halaman : </span><select onchange=\"pagingViewListLoad(this[this.selectedIndex].value,'$this->items_per_page$this->querystring');return false\">$option</select>\n";
	}
