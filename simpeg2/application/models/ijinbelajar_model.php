...
class Ijinbelajar_model extends CI_Model{
	...
	public function daftarib($status_approve = NULL)
	{
		$query = "select 
					...
					concat(pegawai.gelar_depan,' ',pegawai.nama,concat(', ',pegawai.gelar_belakang)), concat(pegawai.gelar_depan,' ',pegawai.nama) ) as nama,
					nip_baru,pangkat_gol,jabatan,nama_baru,pegawai.id_pegawai as idp, jfu_master.nama_jfu 
					from ijin_belajar 
					inner join pegawai on pegawai.id_pegawai=ijin_belajar.id_pegawai 
					left join formasi on formasi.id_formasi = ijin_belajar.id_formasi
					left join jfu_master on jfu_master.id_jfu = formasi.id_jfu
   ...
   return $query->row();
	...
	{
		$sql = "select file_name from berkas inner join isi_berkas on isi_berkas.id_berkas=berkas.id_berkas
              where id_pegawai=$idp and ket_berkas like '$tp' and id_kat=20 AND file_name LIKE '%.pdf'
              ORDER BY id_isi_berkas DESC LIMIT 1";
	...
	$tp--;
        ...
        $sql = "select file_name from ijin_belajar inner join isi_berkas on ijin_belajar.ijazah=isi_berkas.id_berkas where id_pegawai=$idp and tingkat_pendidikan=$tp AND id_berkas<> 0";
		$query = $this->db->query($sql);
    ...
    public function ambil($idp,$tp)
   {
    ...
    public function proses_kembali($idp,$tp)
    {

        $user = $this->session->userdata('user')->nama_pendek;
        $skr=date("Y-m-d");
        $query = $this->db->query("update ijin_belajar set approve=2,approved_by='$user',tgl_approve='$skr' where id_pegawai=$idp and tingkat_pendidikan=$tp ");
    }
   
   public function perbaiki($idp,$tp){
       $user = $this->session->userdata('user')->nama_pendek;
       $skr=date("Y-m-d");
       $query = $this->db->query("update ijin_belajar set approve=7,approved_by='$user',tgl_approve='$skr' where id_pegawai=$idp and tingkat_pendidikan=$tp ");
   }
