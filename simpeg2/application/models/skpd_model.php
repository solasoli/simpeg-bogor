...
class Skpd_model extends CI_Model{
	...
	$lpendidikan = $this->input->post('lpendidikan');
	$lpengalaman = $this->input->post('lpengalaman');
	$lpelatihan = $this->input->post('lpelatihan');
	$ladministrasi = $this->input->post('ladministrasi');
	$pendidikan = $this->input->post('pendidikan');
	$pengalaman = $this->input->post('pengalaman');
	$pelatihan = $this->input->post('pelatihan');
	$administrasi = $this->input->post('administrasi');
	$skor=($pendidikan+$pelatihan+$pengalaman+$administrasi)*0.25;
	$this->db->query("insert into ipasn_kompetensi (jabatan,id_j,id_unit_kerja,fungsi,nama,nip,pendidikan,pelatihan,pengalaman,administrasi,flag_pendidikan,flag_pelatihan,flag_pengalaman,flag_administrasi,skor) values ('$jabatan',$idj,$skpd,'$tupoksi','$nama','$nip','$lpendidikan','$lpelatihan','$lpengalaman','$ladministrasi',$pendidikan,$pelatihan,$pengalaman,$administrasi,$skor)");
	}
	
		public function update_ipasn($idj)
	{
	$idj= $this->input->post('idj');
	$skpd= $this->input->post('skpd');
	$nip= $this->input->post('nip');
	...
	$administrasi = $this->input->post('administrasi');
	$skor=($pendidikan+$pelatihan+$pengalaman+$administrasi)*0.25;
	$this->db->query("update ipasn_kompetensi set flag_pendidikan=$pendidikan,flag_pelatihan=$pelatihan,flag_pengalaman=$pengalaman,flag_administrasi=$administrasi,skor=$skor where id_j=$idj");
	}
