...
<?php
mysqli_connect('localhost', 'root', 'koplak123'); mysqli_select_db('simpeg_20141231');
$mysqli = new mysqli("simpeg.db.kotabogor.net","simpeg","Madangakara2017","simpeg_20141231");
extract($_GET);

$rs = mysqli_query($mysqli,"SELECT nama_baru FROM unit_kerja WHERE id_unit_kerja = $id");
...
$result = $mysqli->query("CALL PRC_VIEW_DUK($id)");
$i = 1;
if($result):?>
    ...
    <?php while ($row = $result->fetch_array()): ?>
		<tr class="table-small">
			...
			$nip=$row[nip];
			$qpaksa=mysqli_query($mysqli,"select jenjab,jabatan,id_pegawai,id_j,pangkat_gol from pegawai where nip_lama='$nip' or nip_baru='$nip'");
			$paksa=mysqli_fetch_array($qpaksa);
			$qsk=mysqli_query($mysqli,"select * from sk where id_pegawai=$paksa[2] and id_kategori_sk=6");
			$cpns=mysqli_fetch_array($qsk);
			$potong=explode(",",$cpns[7]);					
			
			 ?></td>
			
			<td><?php echo $paksa[4]; ?></td>
			<td style="min-width:65px"><?php 
			
			$qkp=mysqli_query($mysqli,"select date_format(tmt, '%d-%m-%Y') from sk where id_pegawai=$paksa[2] and id_kategori_sk=5 and keterangan like '$row[golongan]%'");
			...
			*/
			...
			if($kp){
				...
				$qpns = mysqli_query($mysqli,"select date_format(tmt, '%d-%m-%Y') from sk where id_pegawai=$paksa[2] and (id_kategori_sk=6)  and keterangan like '$row[golongan]%' order by tmt desc");
			...
			echo("$paksa[1]");
			?></td>
			<td style="min-width:65px"><?php 	$qj=mysqli_query($mysqli,"select tmt from sk where id_pegawai=$paksa[2] and id_kategori_sk=10 and id_j=$paksa[3]");
			
			if($qj){
				$kj=mysqli_fetch_array($qj);
				$t2=substr($kj[0],0,4);
				$b2=substr($kj[0],5,2);
				$th2=substr($kj[0],8,2);
				if($t2>0)
			...
			<td><?php 
			$qpen=mysqli_query($mysqli,"select * from pendidikan where id_pegawai=$paksa[2] order by level_p"); 
			...
			<td style="min-width:65px"><?php 
				$pTahun = substr($row[tgl_lahir], 0, 4);
				$pBulan = substr($row[tgl_lahir], 5, 2);
				$pTanggal = substr($row[tgl_lahir], 8, 2);
				echo "$pTanggal-$pBulan-$pTahun"  ?></td>
			<td>&nbsp;</td>
		</tr>
	<?php endwhile; ?>
	</tbody>
