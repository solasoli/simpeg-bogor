...
$qjum=mysqli_query($mysqli," SELECT count(*) FROM current_lokasi_kerja inner join pegawai on pegawai.id_pegawai=current_lokasi_kerja.id_pegawai where flag_pensiun=0 and id_unit_kerja $var ");
$jum=mysqli_fetch_array($qjum);



//sd
$qsd=mysqli_query($mysqli," select sum(jum) from (SELECT COUNT(*) as jum,c.id_unit_kerja,  pd.min_level_p
...
WHERE pd.min_level_p = 9
    AND u.id_skpd $var
GROUP BY c.id_unit_kerja) as sd");
...
//smp
$qsmp=mysqli_query($mysqli," select sum(jum) from (SELECT COUNT(*) as jum,c.id_unit_kerja,  pd.min_level_p
FROM current_lokasi_kerja c
INNER JOIN 
(
    SELECT pd.id_pegawai, MIN(pd.level_p) AS 'min_level_p'
...
INNER JOIN unit_kerja u ON u.id_unit_kerja = c.id_unit_kerja
WHERE pd.min_level_p = 8
    AND u.id_skpd $var
GROUP BY c.id_unit_kerja) as smp");
