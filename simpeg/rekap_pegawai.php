...
<h2> REKAPITULASI PEGAWAI PEMERINTAH KOTA BOGOR Per <?php echo date('F Y');?> </h2>
</br>
...
<?php
$result = mysqli_query($mysqli,"select trim(pangkat_gol) as pangkat_gol,
...
group by trim(pangkat_gol) with rollup 
  ");
...
<?php
$result = mysqli_query($mysqli,"SELECT LEFT(NOW(), 10) AS tanggal, trim(pangkat_gol) as pangkat_gol, COUNT(pangkat_gol) AS jumlah 
FROM pegawai 
Left join current_lokasi_kerja clk on clk.id_pegawai = pegawai.id_pegawai
where flag_pensiun = 0

GROUP BY trim(pangkat_gol) ");
?>
<table class="table table-bordered">
<thead>
<tr>
