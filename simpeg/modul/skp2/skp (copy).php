...
class Skp{
	...
	public function get_dinilai($pegawai){
		...
		if($pegawai->id_j == NULL) {			
			//return $this->pegawai->dinilai_ext($pegawai);
			return false;
		}
	...
	public function get_dinilai_ext($pegawai){
	...
	public function get_penilai($pegawai){
		...
		if($pegawai->id_j !=NULL && $pegawai->jenjab == 'Struktural'){ 
			...
			if($result){
				...
				echo "penilai dari ".$pegawai->nama." tidak ditemukan";
			}
		...
		if($jft->jabatan == "Guru"){
			...
			/*penilai guru disini (Kepala sekolah) */
				if($kepsek = $obj_pegawai->get_by_kepsek_di($jft->id_unit_kerja)){
					...
					echo "Kepala Sekolah tidak terdefinisi";
				}
			}
		}else{
			//jft yang lainnya
			if(strpos($jft->jabatan,"Pengawas Sekolah") !== FALSE ||
				...
				$uk = new Unit_kerja();
				//echo $uk->get_kepala($jft->id_unit_kerja);
				//echo $jft->id_unit_kerja;
				return $obj_pegawai->get_obj($uk->get_kepala($jft->id_unit_kerja)->id_pegawai);
				//return $jft;
			}
