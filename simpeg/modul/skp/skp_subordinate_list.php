		...
		<table class="clearfix table table-bordered table-striped table-hover">
			...
			<tbody>
				<?php
					$r = $_GET['t'];
				if($r=='review'){
					$list_bawahan = $skp->get_dinilai($pegawai);
					...
					if($list_bawahan){
					$x = 1;
					while($each_bawahan = mysqli_fetch_object($list_bawahan)){

					//$bawahan = $obj_pegawai->get_obj($obj_pegawai->get_obj($each_bawahan->id_pegawai)->id_pegawai);
					$bawahan = $obj_pegawai->get_obj($each_bawahan->id_pegawai);
				...
				<tr>
					...
					<td class="hidden-print">
						<a href="index.php?page=los&t=<?php echo $_GET['t']?>&idp=<?php echo $each_bawahan->id_pegawai ?>" class="btn btn-info btn-sm">lihat</a>
					</td>
				</tr>
				<?php }}else{ ?>
				<tr>
