...
<nav>
	<ol class="cd-multi-steps text-bottom count">
		...
		<li class="visited"><strong>Realisasi</strong></li>
	...
	<div class="panel-body">
		...
		<table class="clearfix table table-bordered">
			...
			<tr >
				...
				<!-- end of target -->
				<!-- realisasi -->
				<td class="text-center"><?php echo $target->real_angka_kredit != NULL ? $target->real_angka_kredit : '-' ?></td>
				<td class="text-center"><?php echo $target->real_kuantitas != NULL ? $target->real_kuantitas." ".$target->kuantitas_satuan : '-'?></td>
				<td class="text-center"><?php echo $target->real_kualitas != NULL ? $target->real_kualitas : '-'?></td>
				<td class="text-center"><?php echo $target->real_waktu != NULL ? $target->real_waktu." ".$target->waktu_satuan : '-' ?></td>
				<td class="text-center"><?php echo $target->real_biaya != NULL ? $format->currency($target->real_biaya) : '-' ?></td>
				<td class="text-center"><?php echo $target->hitung_nilai != NULL ? $target->hitung_nilai : '-' ;?></td>
				<td class="text-center"><?php echo $target->nilai_capaian != NULL ? $target->nilai_capaian : '-'; ?></td>
				<?php if($pegawai->skp_block == 0){ ?>
				<td style="padding-right:0px" class="hidden-print">
					...
					<a data-toggle="tooltip" title="reset realisasi, penghitungan dan nilai capaian SKP" onclick="reset_penghitungan(<?php echo $target->id_skp_target ?>)" class="btn btn-danger btn-xs hidden btn-revisi">reset</a>
				</td>
				<?php } ?>
			</tr>
			<?php } ?>
			...
			<?php
				...
				while($tambahan = mysqli_fetch_object($list_tambahan)){
			?>
			<tr>
				<td ></td>
...
<script>
	...
	function reset_penghitungan(idtarget){
		//alert(idtarget);
		$.post("skp.php",{aksi: "resetPenilaian", idtarget:idtarget})
		 .done(function(data){
			 //alert(data);
			 window.location.reload();
		 });
