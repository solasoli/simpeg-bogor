...
<table width="100%" border="0" cellpadding="3" cellspacing="0" class="hurup table">
            ...
            $arrGol = array();
            $qkg=mysql_query("SELECT * FROM `golongan`");
            while($dag=mysql_fetch_array($qkg))
            {
                $arrGol[] = array('val'=>$dag[1], 'title'=>$dag[1].' ('.$dag[2].')');
            }

			$k=1;
			$qsk=mysql_query("select *, FCN_PARSE_KETERANGAN_SK(keterangan, 1) AS gol,
