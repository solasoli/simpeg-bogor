...
if(isset($idel))
{
mysqli_query($mysqli,"delete from proper where id_proper=$idel");
mysqli_query($mysqli,"delete from proper_tujuan where id_proper=$idel");
}

if(isset($judul) and isset($desc))
{
	$qjab=mysqli_query($mysqli,"select jabatan.jabatan from jabatan inner join pegawai on jabatan.id_j=pegawai.id_j where pegawai.id_pegawai=$_SESSION[id_pegawai]");
$jab=mysqli_fetch_array($qjab);

mysqli_query($mysqli,"insert into proper (id_pegawai,id_jabatan,judul,deskripsi,id_mentor,id_jabatan_mentor,tingkat,tahun) values ($_SESSION[id_pegawai],'$peserta->id_j','$judul','$desc',$id_penilai,'$mentor->id_j','$pim',$tahun)");
		...
		<table class="clearfix table table-bordered" id="listskp">
			...
			<tbody>
				<?php
					...
					$qcek=mysqli_query($mysqli,"select count(*) from proper where id_pegawai=$_SESSION[id_pegawai]");
					$cek=mysqli_fetch_array($qcek);

					$qcek2=mysqli_query($mysqli,"select count(*) from proper where id_mentor=$_SESSION[id_pegawai]");
					$cek2=mysqli_fetch_array($qcek2);
...
if($cek[0]>0)
{
$qpro=mysqli_query($mysqli,"select * from proper where  id_pegawai=$_SESSION[id_pegawai]");
					while($pro=mysqli_fetch_array($qpro)){
				?>
				<tr>
					<td><?php echo $t++ ;  ?>.</td>
...
$qpro=mysqli_query($mysqli,"select * from proper where  id_mentor=$_SESSION[id_pegawai]");
					while($pro=mysqli_fetch_array($qpro)){
				?>
				<tr>
					<td><?php echo $t++ ;  ?>.</td>
					<td><?php echo ($pro['judul']);
                        ...
                        <td align="center"><?php  echo $pro[7]; ?> </td>
                         <td align="center"><?php  echo $pro[8]; ?> </td>
					<td class="hidden-print">
						<a href="index.php?page=ubah_data&idp=<?php echo $pro[0]; ?>" class="btn btn-primary btn-xs">BUKA</a>
