...
mysql_query("update proper_tujuan set status='$status',alasan='$alasan' where id_proper=$idp and jenis_jangka like 'pendek'");
}
else if($jangka=='menengah')
{
mysql_query("update proper_tujuan set status='$status',alasan='$alasan' where id_proper=$idp and jenis_jangka like 'menengah'");
}
else
{
mysql_query("update proper_tujuan set status='$status',alasan='$alasan' where id_proper=$idp and jenis_jangka like 'panjang'");
...
<nav>
		...
		<li class=""><a href="index.php?page=ubah_data&idp=<?php echo $idp; ?>">Data</a></li>
		<li class=""><a href="index.php?page=unggah&idp=<?php echo $idp; ?>">Upload PDF</a></li>
         <li class="visited"><strong>Monitoring</strong></li>
...
$th2=substr($pendek[4],0,4);
					...
					<div class="form-group">
						...
						<input type="text" readonly="readonly" name="tujuan" id="tujuan" class="form-control"  <?php if($cek1[0]>0) echo (" value='".$pendek[3]."' "); ?> >
					</div>
					
					<div class="form-group">
						<label for="periode_awal">Target Waktu Jangka Pendek</label>
						<div class="form-inline">
						  Tgl: <select name="t1" id="t1" disabled="disabled" ><?php 
						  for($i=1;$i<=31;$i++)
						  {
							  if($i<10)
							  {
						   ...
						   ?> </select>
                           <select name="b1" id="b1" disabled="disabled" ><?php 
						  for($j=1;$j<=12;$j++)
						  {
						   ...
						   ?> </select>
                           <select name="th1" id="th1" disabled="disabled" ><?php 
                    ...
                    <div class="form-group">
						<label for="jabatan_pegawai">Status</label>
						<select name="status" id="status"><option value="berjalan"
                        
                         <?php if($cek1[0]>0 and $pendek[5]=='berjalan' ) echo (" selected=selected "); ?>
                        > Berjalan </option><option value="tidak berjalan"  
                          <?php if($cek1[0]>0 and $pendek[5]=='tidak berjalan' ) echo (" selected=selected "); ?>
                        > Tidak Berjalan </option> </select>
					</div>
					
					   <div class="form-group">
						<label for="jabatan_pegawai">alasan</label>
						<input type="text" name="alasan" id="alasan" class="form-control" <?php if($cek1[0]>0) echo (" value='".$pendek[6]."' "); ?>>
					</div>
                    <input type="hidden" name="jangka" id="jangka" value="pendek" />
                      <input type="hidden" name="page" id="page" value="monitoring" />
                        <input type="hidden" name="idp" id="idp" value="<?php echo $idp; ?>" />
<button id="btnPeriode" class="btn btn-primary" onclick="submit1();">AJUKAN</button>
				</form>
...
$th3=substr($menengah[4],0,4);
					...
					<div class="form-group">
						<label for="gol_pegawai">Tujuan</label>
						<input type="text" readonly="readonly" name="tujuan" id="tujuan" class="form-control"  <?php if($cek2[0]>0) echo (" value='".$menengah[3]."' "); ?>>
					</div>
					...
					<select name="status" id="status"><option value="berjalan"
                         ...
                         <?php if($cek2[0]>0 and $menengah[5]=='berjalan' ) echo (" selected=selected "); ?>
                        > Berjalan </option><option value="tidak berjalan"  
                          <?php if($cek2[0]>0 and $menengah[5]=='tidak berjalan' ) echo (" selected=selected "); ?>
                        > Tidak Berjalan </option> </select>
					</div>
					
					   <div class="form-group">
						<label for="jabatan_pegawai">alasan</label>
						<input type="text" name="alasan" id="alasan" class="form-control" <?php if($cek2[0]>0) echo (" value='".$menengah[6]."' "); ?>>
					</div>
                    <input type="hidden" name="jangka" id="jangka" value="menengah" />
                    <input type="hidden" name="page" id="page" value="monitoring" />
                    <input type="hidden" name="idp" id="idp" value="<?php echo $idp; ?>" />
<button id="btnPeriode" class="btn btn-primary" onclick="submit2();">AJUKAN</button>
				</form>
			</div>
		</div>
	</div>
...
?>
					...
					<div class="form-group">
						<label for="gol_pegawai">Tujuan</label>
						<input readonly="readonly" type="text" name="tujuan" id="tujuan" class="form-control" <?php if($cek3[0]>0) echo (" value='".$panjang[3]."' "); ?>>
					</div>
					
					<div class="form-group">
						<label for="periode_awal">Target Waktu Jangka Panjang <?php  ?></label>
						<div class="form-inline">
						  Tgl: <select disabled="disabled" name="t1" id="t1" disabled="disabled" ><?php 
						  for($i=1;$i<=31;$i++)
						  {
							  if($i<10)
							  {
                    ...
                    <div class="form-group">
						...
						<select name="status" id="status"><option value="berjalan"
                         ...
                         <?php if($cek3[0]>0 and $panjang[5]=='berjalan' ) echo (" selected=selected "); ?>
                        > Berjalan </option><option value="tidak berjalan"  
                          <?php if($cek3[0]>0 and $panjang[5]=='tidak berjalan' ) echo (" selected=selected "); ?>
                        > Tidak Berjalan </option> </select>
					</div>
					
					   <div class="form-group">
						<label for="jabatan_pegawai">alasan</label>
						<input type="text" name="alasan" id="alasan" class="form-control" <?php if($cek3[0]>0) echo (" value='".$panjang[6]."' "); ?>>
					</div>
                    <input type="hidden" name="jangka" id="jangka" value="panjang" />
                    <input type="hidden" name="page" id="page" value="monitoring" />
                    <input type="hidden" name="idp" id="idp" value="<?php echo $idp; ?>" />
<button id="btnPeriode" class="btn btn-primary" onclick="submit3();">AJUKAN</button>
				</form>
			</div>
		</div>
	</div>
