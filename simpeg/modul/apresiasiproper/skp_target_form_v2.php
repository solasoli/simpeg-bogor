	...
	<div class="panel-body">
		<table class="clearfix table table-bordered" id="skp_target_table">
			...
			<?php 
				...
				$unsur_sql = mysql_query("select distinct unsur from skp_target where id_skp = ".$_GET['idskp']);
				while($unsur = mysql_fetch_object($unsur_sql)){
				$uns = $unsur->unsur ? $unsur->unsur : "utama";
					
			?>
				<tr>
					<td></td>
					<td colspan="8"><strong><?php echo strtoupper($unsur->unsur) ?></strong></td>
				</tr>
			
			
			<?php
				$list_target = $skp->get_target($_GET['idskp'],$uns);
				$x = 1;
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form role="form" class="form-horizontal" id="formTambahUraian">
					<?php //if($pegawai->jenjab == 'Fungsional'){ ?>
						<div class="form-group">
							<label for="kecamatan" class=" col-sm-3 control-label">Unsur: </label>
							<div class="col-sm-9">
							<select class="form-control" id="unsur" name="unsur" >								
								<option value="utama" selected="selected">Unsur Utama</option>								
