<h2>PENJAGAAN KENAIKAN GAJI BERKALA</h2>
<hr/>
<fieldset>
<label>SKPD</label>
<select id="cboSkpd">
...
$(document).ready(function(){
	...
	$("#btnView").click(function(){
		$("#loading").html('');
		tampil_load();
		$.post('../simpeg2/index.php/kgb/json_nominatif2', {
				idpegawai: '',
				skpd: $("#cboSkpd").val(),
				tahun: $("#cboTahun").val(),
				bulan: $("#cboBulan").val(),
			...
			function(data){
				...
				for(var i = 0; i <= peg.length;i++){
					...
					$("#kgb_list").append("<tr>" +
						...
						"<td>"+ peg[i].gol_usulan +"</td>" +
						"<td>"+ peg[i].tmt_terakhir +"</td>" +
						"<td>"+ peg[i].mk_tahun_terakhir +"</td>" +
						"<td>"+ peg[i].mk_bulan_terakhir +"</td>" +
						"<td>"+ peg[i].dasar_kgb +"</td>" +
						"<td>"+ peg[i].unit +"</td>" +
						"<td>"+ status  + "</td>" +
					"</tr>");
					no++;
