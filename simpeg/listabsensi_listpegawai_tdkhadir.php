...
<table width="100%" border="0" align="center" style="border-radius:5px;"
    ...
    <?php while($row = mysqli_fetch_array($query_row)){ ?>
        ...
        <tr>
            <td></td>
            <td colspan="6">Jumlah: <span style="font-weight: bold;"><?php echo $row['jml_hari'].' Hari ( C: '.$row['C'].
                    ' DL: '.$row['DL'].' DI:'.$row['DI'].' I:'.$row['I'].' S:'.$row['S'].' TK:'.$row['TK'].' )'; ?></span>
                <br>Waktu: <span style="font-style: italic"><?php echo $row['hist_absensi']; ?></span>
