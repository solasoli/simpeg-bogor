...
header("Content-Disposition:attachment;filename=DUK_".str_replace(",","",str_replace(" ","_",$unor)).".xls");

?>
<h3>DAFTAR URUT KEPANGKATAN<br>
	...
	<tbody>	
		<?php
			...
			if(isset($_REQUEST['opd']) ){
				...
				if(isset($_REQUEST['uk'])){					
					$result = $duk->get_duk($_REQUEST['opd'],$_REQUEST['uk']);
				}else{
					$result = $duk->get_duk($_REQUEST['opd']);
				}				
