...
<h2>DAFTAR PENDIDIKAN PEGAWAI <br><?php echo $unit_kerja->get_unit_kerja($unit['id_skpd'])->nama_baru ?><a href="#" class="btn btn-primary  hidden-print pull-right" onclick="window.print()">cetak</a></h2>
...
<table id="list_pegawai" cellspacing="0" width="100%" class="display table table-striped">
	<thead>
		<tr>
			...
			<th class="hidden-print">Ijazah</th>
	...
	<tbody>
		...
		<form name="formos<?php echo $r; ?>" id="formos<?php echo $r; ?>" enctype="multipart/form-data" >
			...
			<td><?php echo("$bata[12] $bata[13] $bata[14] "); ?></td>
		
			<td>
			<?php if($bata[15]==0){
				echo("<input type=file name=fija id=fija$r />");	
				echo("<input type=hidden name=idpen id=idpen$r value=$bata[16] />");
			}
			else
			{
				$qli=mysqli_query($mysqli,"select file_name from isi_berkas where id_berkas=$bata[15]");
				$lihat=mysqli_fetch_array($qli);
				$nf=basename($lihat[0]);
				$ext = pathinfo("$lihat[0]", PATHINFO_EXTENSION);	  
				if($ext!="pdf")	  {
					echo("<a href=berkas.php?idb=$bata[15] target=_blank>Preview </a>");
					
				}else{
					echo "<a href=../simpeg/Berkas/$nf target=_blank>Preview ..</a>";
