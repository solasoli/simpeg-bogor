...
$query = $oCuti->rekap_cuti_semua_opd($thn, $idstatus);
$i = 0;
?>

<?php if ($query->num_rows > 0): ?>
    <table id="tblRekapAllOPD" width="100%" border="0" align="center" style="border-radius:5px; border-bottom: double 2px rgba(71,71,72,0.35);"
    class="table table-bordered table-hover table-striped">
        ...
        <?php while ($row_data = $query->fetch_array(MYSQLI_BOTH)) {
            $i++; ?>
            <?php if($row_data['opd']=='Total'): ?>
                <tr>
                    <td style="background-color: rgba(67,135,131,0.1);"></td>
                    <td style="background-color: rgba(67,135,131,0.1);"><?php echo $row_data['opd']?></td>
