...
extract($_POST);
extract($_GET);
$q=mysqli_query($mysqli,"select nama,nip_baru,id_pegawai,right(left(nip_baru,14),2)  from pegawai where (right(left(nip_baru,13),1)='1' or right(left(nip_baru,14),1)>4   ) and flag_pensiun=0 and jenjab like 'struktural' and length(nip_baru)=18 and (left(curdate(),4)-right(left(nip_baru,12),4))%4=0 and left(curdate(),4)-right(left(nip_baru,12),4)>=4 order by  right(left(nip_baru,14),2)
");
