...
<?php
include("konek.php");
$q=mysqli_query($mysqli,"select * from pegawai inner join jabatan on jabatan.id_j=pegawai.id_j inner join pendidikan_terakhir on pegawai.id_pegawai=pendidikan_terakhir.id_pegawai where flag_pensiun=0 order by jabatan.eselon,level_p");
?>
<table id="list_pegawai"  class="table table-bordered display" width="100%" border="0" cellspacing="0" cellpadding="5">
  ...
  while($data=mysqli_fetch_array($q))
  {
	...
	echo("$tmt[0]");
	 echo("</td>
    <td>");
	$q2=mysqli_query($mysqli,"select jabatan.jabatan,tmt from sk inner join jabatan on sk.id_j=jabatan.id_j where sk.id_pegawai=$data[0] and id_kategori_sk=10 order by tmt desc");
