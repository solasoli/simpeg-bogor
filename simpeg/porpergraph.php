...
$(function () {
    $('#gol').highcharts({
        ...
        title: {
			...
			{
			$qunit=mysql_query("select id_skpd from unit_kerja where id_unit_kerja=$_SESSION[id_unit]");
			$unit=mysql_fetch_array($qunit);
			...
			$q=mysql_query("select count(*) from pegawai $where");
			$itung=mysql_fetch_array($q);
...
$qg=mysql_query("SELECT count( * ) , tahun
FROM proper 
GROUP BY tahun");
...
?>		
        ...
        series: [{
			...
			<?php
			$i=1;
			while($data=mysql_fetch_array($qg))
			{
