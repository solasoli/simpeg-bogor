<?php
include("konek.php");
$q=mysqli_query($mysqli,"select nama,gelar_depan,gelar_belakang,pegawai.id_pegawai from pegawai inner join pendidikan_terakhir on pendidikan_terakhir.id_pegawai=pegawai.id_pegawai where flag_pensiun=0 and  ( gelar_depan like 'dr%') and level_p>1");
extract($_POST);
if(isset($id))
{
...
<table width="500" border="0" align="center" cellpadding="5" cellspacing="0">
 ...
 $qd=mysqli_query($mysqli,"select count(*) from pendidikan where id_pegawai=$data[3] and level_p<2");
     $dek=mysqli_fetch_array($qd);
	 if($dek[0]==0)
	 {
	echo("<form name=form$i id=form$i method=post action=fixs3.php>");
	echo("<tr>
    <td>$i</td>
    <td nowrap>$data[0]</td>
    <td nowrap>$data[1]</td>
	...
	echo("<select name=pen id=pen>");
		...
		$q1=mysqli_query($mysqli,"select tingkat_pendidikan,level_p from pendidikan where level_p<2 and level_p>0 group by level_p order by level_p desc ");
