...
<table width="100%" border="1" cellpadding="3" cellspacing="0" class="hurup">
	...
	while($cu=mysqli_fetch_array($qsk))
	{
	  ...
	  <td>$k</td>
	  <td>"); ?>
	  <select name="a<?php echo($k); ?>" id="a<?php echo($k); ?>" style="width:160px;" >
	  <?php
      ...
      ?>
        ...
        <td>
            <select name="gol_sk<?php echo($k); ?>" id="gol_sk<?php echo($k); ?>" style="width:160px;" >
	...
	if($cu[10]==NULL or $cu[10]==0)
	  ...
	  if($cu[2]==5 or $cu[2]==8 or $cu[2]==60 )
	  {
		  $qtol=mysqli_query($con,"select file_name from isi_berkas where id_berkas=$cu[10]");
		  $tul=mysqli_fetch_array($qtol);
		  if($cu[2]==5)
		  {
	  ?>
      <a href=#
                                                               name="btnWA<?php echo $cu[0]; ?>"
                                                               id="btnWA<?php echo $cu[0]; ?>"
                                                               class="btn btn-success btn-sm"
                                                               style="font-weight: bold;width: 100%; background-color: #57a957;color: white; width:77px; border-radius:5px;"><span class="icon-mail-2 on-left"></span>Notif WA</strong></a>
                                                               
                                                                   <script type="text/javascript">
                                                            $("#btnWA<?php echo $cu[0]; ?>").click(function () {
                                                                ...
                                                                $.post('https://eu14.chat-api.com/instance25721/message?token=32r2xt8sm5oxb5nx',
                                                                {
                                                                    ...
                                                                    "body": "Yth. Bpk/Ibu Pegawai Pemkot Bogor SK Kenaikan Pangkat <?php echo $cu['gol'];  ?> a.n. <?php echo stripslashes($ata['nama']);?> sudah tersedia, anda dapat mengunduhnya di akun simpeg anda atau Klik tautan http://arsipsimpeg.kotabogor.go.id/simpeg/berkas/<?php echo $tul[0]; ?> untuk mengunduh. Berkas Kenaikan Pangkat dapat diambil langsung di Anjungan Layanan Terpadu (ALAPADU) Kantor BKPSDA Kota Bogor, Terimakasih"
                                                                },
                                                                function(data){
                                                                    if(data.sent==true){
      ...
      <?php
		  ...
		  else if($cu[2]==8)
		  {
              ...
              <a href=#
                                                            ...
                                                            $("#btnWA<?php echo $cu[0]; ?>").click(function () {
                                                                ...
                                                                {
                                                                    "phone": '<?php echo '62'.(substr($ponsel,1,strlen($ponsel)-1));?>',
                                                                    "body": "Yth. Bpk/Ibu Pegawai Pemkot Bogor SK Pensiun  a.n. <?php echo stripslashes($ata['nama']);?> sudah tersedia, anda dapat mengunduhnya dengan klik tautan http://arsipsimpeg.kotabogor.go.id/simpeg/berkas/<?php echo $tul[0]; ?> untuk mengunduh. Berkas Pensiun dapat diambil langsung di Anjungan Layanan Terpadu (ALAPADU) Kantor BKPSDA Kota Bogor, Terimakasih."
                                                                },
                                                                function(data){
                                                                    if(data.sent==true){
                                                                        alert("Pesan WhatsApp terkirim");
		   ...
		   else if($cu[2]==60)
		   {
               ...
               <a href=#
                                                            ...
                                                            $("#btnWA<?php echo $cu[0]; ?>").click(function () {
                                                                ...
                                                                {
                                                                    "phone": '<?php echo '62'.(substr($ponsel,1,strlen($ponsel)-1));?>',
                                                                    "body": "Yth. Bpk/Ibu Pegawai Pemkot Bogor Surat Tanda Lulus Ujian Penyesuaian Kenaikan Pangkat a.n. <?php echo stripslashes($ata['nama']);?> sudah tersedia, anda dapat mengunduhnya dengan klik tautan http://arsipsimpeg.kotabogor.go.id/simpeg/berkas/<?php echo $tul[0]; ?> untuk mengunduh. Berkas Kelulusan dapat diambil langsung di Anjungan Layanan Terpadu (ALAPADU) Kantor BKPSDA Kota Bogor, Terimakasih."
                                                                },
                                                                function(data){
                                                                    if(data.sent==true){
                                                                        alert("Pesan WhatsApp terkirim");
        ...
        <?php
            if($nama_sknya=='NIP Baru'){?>
                &nbsp; | &nbsp; <a href="https://wa.me/<?php echo '62'.(substr($ponsel,1,strlen($ponsel)-1)); ?>?text=Yth. Bpk/Ibu Pegawai Pemkot Bogor a.n <?php echo $namap;?> diberitahukan bahwa SK Konversi NIP sudah terbit. Terimakasih. BKPSDA Kota Bogor" target="_blank">Kirim pesan via WA <?php echo($ponsel); ?></a>
        <?php
            }elseif($nama_sknya=='SK Sumpah Janji PNS'){?>
                &nbsp; | &nbsp; <a href="https://wa.me/<?php echo '62'.(substr($ponsel,1,strlen($ponsel)-1)); ?>?text=Yth. Bpk/Ibu Pegawai Pemkot Bogor a.n <?php echo $namap;?> diberitahukan bahwa Berita Acara Sumpah Janji PNS sudah terbit. Terimakasih. BKPSDA Kota Bogor" target="_blank">Kirim pesan via WA <?php echo($ponsel); ?></a>
        <?php }

		if($cu[2]==7) echo ("<br>STTPL<br><input type=text name=sttpl$k id=sttpl$k value='".$cu['catatan']."'/><br>
		No.Dokter<br><input type=text name=dokter$k id=dokter$k value='".$cu['keterangan']."'/>
		");
  ...
  echo("</tr>");
		...
		$sql = "select berkas.nm_berkas, berkas.id_berkas, file_name
                from berkas INNER JOIN isi_berkas ON berkas.id_berkas = isi_berkas.id_berkas
                where id_pegawai=$ata[0] and id_kat!=2";

	$qberkas=mysqli_query($con,$sql);



	while($berkas=mysqli_fetch_array($qberkas))
	{
        //$sql = "select id_isi_berkas, file_name from isi_berkas where id_berkas=".$berkas['id_berkas'];
        //$qli=mysqli_query($con,$sql);
		//$lihat=mysqli_fetch_array($qli);
		$nf=basename($berkas[2]);
		$ext = pathinfo("$berkas[2]", PATHINFO_EXTENSION);
	   if($ext!="pdf")
	$link="<a href=berkas.php?idb=$berkas[1]  target=_blank>Preview</a>";
	  else
	  $link="<a href=http://103.14.229.15/simpeg/berkas/$nf target=_blank>Preview </a></td>";
		echo(" <tr>
		<td>  $k</td>
		<td> $berkas[0]
	</td>
		...
		<td>
            <?php
                if($berkas[0]=='Karis/Karsu'){
                    ?>
            <a href="https://wa.me/<?php echo '62'.(substr($ponsel,1,strlen($ponsel)-1)); ?>?text=Yth. Bpk/Ibu Pegawai Pemkot Bogor a.n <?php echo $namap;?> diberitahukan bahwa Karis/Karsu sudah terbit. Terimakasih. BKPSDA Kota Bogor" target="_blank">Kirim pesan via WA <?php echo($ponsel); ?></a>

            <?php    }elseif($berkas[0]=='Kartu Pegawai'){ ?>
                    <a href="https://wa.me/<?php echo '62'.(substr($ponsel,1,strlen($ponsel)-1)); ?>?text=Yth. Bpk/Ibu Pegawai Pemkot Bogor a.n <?php echo $namap;?> diberitahukan bahwa KARPEG sudah terbit. Terimakasih. BKPSDA Kota Bogor" target="_blank">Kirim pesan via WA <?php echo($ponsel); ?></a>
                <?php    }elseif($berkas[0]=='Kartu Pegawai'){ ?>
                <?php } ?>
        </td>
		<?php ("<td>   </td>
	  ...
	  $qks=mysqli_query($con,"SELECT * FROM `kat_berkas`");
	    ...
	    <script type="text/javascript">
            ...
            function getValCboJsk(sel){
				...
				}else if(sel.value == 34 || sel.value == 35 || sel.value == 11) {
					var elementExists = document.getElementById("divSkpd");
					if (typeof(elementExists) != 'undefined' && elementExists != null) {
						var d = document.getElementById('kol3');
						var olddiv = document.getElementById('divSkpd');
