<?php if (isset($drop_data_list) and sizeof($drop_data_list) > 0 and $drop_data_list != ''){ ?>
                    ...
                    <tr>
                        <td>
                            <div id="dvListAct<?php echo $lsdata->id_knj_kegiatan;?>">
                                <?php if($this->session->userdata('nip')==$lsdata->atsl_nip): ?>
                                    <?php if($lsdata->atsl_nama_plh == ''): ?>
                                ...
                                <?php if($this->session->userdata('nip')==$lsdata->atsl_nip): ?>
                                    <?php if($id_status_knj==1 or $id_status_knj==2 or $id_status_knj==5): ?>
                                        ...
                                        <?php if($lsdata->atsl_nama_plh == ''): ?>
                                            ...
                                            <?php endif; ?>
                                            <?php if($lsdata->approved == ''): ?>
                                                <a href="javascript:void(0);" onclick="proses_persetujuan_aktifitas(1, <?php echo $lsdata->id_knj_kegiatan;?>, '<?php echo $lsdata->id_knj_kegiatan_enc;?>', <?php echo $lsdata->no_urut; ?>);"><span class="mif-checkmark icon"></span> Disetujui</a> &nbsp;
                                                <a href="javascript:void(0);" onclick="proses_persetujuan_aktifitas(2, <?php echo $lsdata->id_knj_kegiatan;?>, '<?php echo $lsdata->id_knj_kegiatan_enc;?>', <?php echo $lsdata->no_urut; ?>);"><span class="mif-pencil icon"></span> Revisi</a> &nbsp;
                                                <a href="javascript:void(0);" onclick="proses_persetujuan_aktifitas(3, <?php echo $lsdata->id_knj_kegiatan;?>, '<?php echo $lsdata->id_knj_kegiatan_enc;?>', <?php echo $lsdata->no_urut; ?>);"><span class="mif-cross icon"></span> Ditolak</a>
                                            <?php else: ?>
                                                <a href="javascript:void(0);" onclick="proses_persetujuan_aktifitas(0, <?php echo $lsdata->id_knj_kegiatan;?>, '<?php echo $lsdata->id_knj_kegiatan_enc;?>', <?php echo $lsdata->no_urut; ?>);"><span class="mif-cancel icon"></span> Batalkan Pemrosesan</a>
                                            <?php endif; ?>
                                        <?php else: ?>
                                            <?php
                                                echo '<span style="color: dodgerblue;">Kegiatan ini diproses oleh Plh.</span><br>'.$lsdata->atsl_nama_plh.' ('.$lsdata->atsl_nip_plh.')<br>'.$lsdata->atsl_jabatan_plh;
                                            ?>
                                        <?php endif; ?>
                                    <?php endif; ?>
                                <?php endif; ?>
    ...
    <script>
        ...
        $("#chkCheckAll").change(function () {
            $("#dvAllListAktifitas input:checkbox").prop('checked', $(this).prop("checked"));
        });

        function refresh_table(){
            var table = $('#tblAktifitas').data('table');
            table.draw(true);
        }

        function proses_persetujuan_aktifitas(idstatus, id_knj_kegiatan, id_knj_kegiatan_enc, no_urut){
            $.confirm({
                title: 'Informasi',
                        ...
                        action: function(){
                            $.alert({
                                ...
                                content: function () {
                                    ...
                                    var ket_approval = $('#txtApproval'+id_knj_kegiatan).val();
                                ...
                                buttons: {
                                    refreshList: {
                                        ...
                                        action: function () {
                                            ...
                                            return $.ajax({
                                                url: "<?php echo base_url($usr)."/get_aktifitas_by_id/";?>",
                                                data: data2,
                                                cache: false,
                                                contentType: false,
                                                processData: false,
                                                ...
                                                $("#dvListAct" + id_knj_kegiatan).html(data);
                                                $("#dvListAct" + id_knj_kegiatan).css("pointer-events", "auto");
                                                $("#dvListAct" + id_knj_kegiatan).css("opacity", "1");
                                                $("#dvListAct" + id_knj_kegiatan).find("script").each(function(i) {
                                                    eval($(this).text());
                                                });
                                            }).fail(function(){
                                                self.setContent('Error...telah terjadi kesalahan');
