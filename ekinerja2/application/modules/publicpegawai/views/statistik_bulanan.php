...
<div class="row" style="margin-bottom: 15px;">
    ...
    <div class="cell-sm-3">
        <select id="ddTahunStatMonth" name="ddTahun" data-role="select" class="cell-sm-12">
            <?php
            $i = 0;
...
<?php if($this->session->userdata('idknj_admin')!=''): ?>
    <div class="row" style="margin-bottom: 15px;">
        ...
        <div class="cell-sm-6">
            <input id="txtPegawai" name="txtPegawai" type="text" placeholder="" style="width:100%;" onkeypress="clearTxtkey();" value="">
...
<script>
    ...
    $("#btnFilter").click(function(){
        ...
        var ddTahun = $('#ddTahunStatMonth').val();
        loadDataStatistikBulanan(idPegawai, ddTahun);
    });

    loadDefaultDataStatistikBulanan();

    function loadDefaultDataStatistikBulanan(){
        if (typeof $('#idPegawai').val() !== 'undefined' && $('#idPegawai').val() !== '' ) {
            var idPegawai = $('#idPegawai').val();
        }else{
            var idPegawai = '<?php echo $this->session->userdata('id_pegawai_enc'); ?>';
        }
        loadDataStatistikBulanan(idPegawai, <?php echo date('Y'); ?>);
    }

    function loadDataStatistikBulanan(idPegawai, ddTahun){
        ...
        var jc = $.confirm({
            ...
            content: function () {
                ...
                return $.ajax({
                    url: "<?php echo base_url('publicpegawai/Statistikpegawai')."/drop_data_statistik_bulanan/";?>",
                    data: data,
                    cache: false,
                    contentType: false,
                    processData: false,
